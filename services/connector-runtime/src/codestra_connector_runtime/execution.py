...
class Connector(ABC):
    ...
    async def readback(self, reference: str, context: ExecutionContext) -> ConnectorResult:
        return ConnectorResult(False, "unsupported", error=ConnectorError("READBACK_UNSUPPORTED", "readback is not supported"))

    async def reconcile(self, reference: str, context: ExecutionContext) -> ConnectorResult:
        return ConnectorResult(False, "unsupported", error=ConnectorError("RECONCILE_UNSUPPORTED", "reconcile is not supported"))

    async def cancel(self, reference: str, context: ExecutionContext) -> ConnectorResult:
        return ConnectorResult(False, "unsupported", error=ConnectorError("CANCEL_UNSUPPORTED", "cancel is not supported"))
...
class ConnectorRegistry:
    ...
    def resolve_operation(self, operation: str, context: ExecutionContext) -> Connector:
        matches = [connector for connector in self._connectors.values() if any(cap.operation == operation for cap in connector.descriptor.capabilities)]
        visible = []
    ...
    async def execute(self, connector_id: str, operation: str, payload: Mapping[str, Any], context: ExecutionContext) -> ConnectorResult:
        ...
        capability = {item.operation: item for item in connector.descriptor.capabilities}.get(operation)
        if capability is None:
            raise ConnectorFailure(ConnectorError("CAPABILITY_UNSUPPORTED", "connector operation is not supported"))
        if capability.effect is not EffectClass.READ_ONLY and not context.effects_allowed:
            raise ConnectorFailure(ConnectorError("EFFECT_DISABLED", "connector effects are disabled by policy"))
        try:
            ...
            raise ConnectorFailure(ConnectorError("PROVIDER_TIMEOUT", "connector execution timed out", retryable=True)) from exc
        except asyncio.CancelledError:
            raise
        except ConnectorFailure:
            raise
        except Exception as exc:
            raise ConnectorFailure(ConnectorError("PROVIDER_ERROR", "connector provider failed", provider_code=type(exc).__name__)) from exc
