...
def validate(environ: Mapping[str, str]) -> None:
    for name in ("DATABASE_URL", "TEST_DATABASE_URL"):
        ...
        if (target.scheme not in {"postgresql", "postgresql+asyncpg"}
                or target.hostname != "127.0.0.1" or target.port is None
    ...
    redis_target = urlsplit(environ.get("REDIS_URL", ""))
    if (redis_target.scheme != "redis" or redis_target.hostname != "127.0.0.1"
            or redis_target.port is None or redis_target.path != "/15"
            or redis_target.query or redis_target.fragment):
