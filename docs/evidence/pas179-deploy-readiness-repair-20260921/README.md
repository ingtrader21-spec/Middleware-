...
`ingtrader21-spec/Infustruction-repo`), and the merged tree passes 8 of the 9 PR-time workflows when executed
locally on Ubuntu with the exact CI commands. Two governance gates are **still red on main after the merge**, both
verified by execution, neither fixable by a one-line patch: (a) the `Production orchestrator contract` check fails
...
|---|---|---|
| `REUSABLE_DEPLOY_READINESS` | **PASS (local Linux)** / hosted **NOT EXECUTED** | `certify-60eff4c.log`: deploy-readiness build-context, container-contract and release-assets suites PASS on tree `e0699d97`; hosted jobs zero-step (billing) |
...
WSL Ubuntu, Python 3.14.4 (CI: ubuntu-24.04 python3 / setup-python 3.11–3.12), PyYAML 6.0.3, pytest 8.4.2,
gitleaks 8.30.1 (sha256 verified). Env mirrored: `GITHUB_REPOSITORY=ingtrader21-spec/Infustruction-repo`,
`GITHUB_REPOSITORY_ID=1350724865`, `EXPECTED_SHA=60eff4c…`, `COMPARISON_SHA=9d32d421…`. Log: `certify-60eff4c.log`.
...
- `certify-60eff4c.log` — local Linux execution of all 9 PR workflows on the merged tree
