...
def test_readback_is_read_only() -> None:
    section = API.split('@router.get("/{request_id}/readback")', 1)[1].split(
...
def test_repair_intent_has_explicit_mutation_route() -> None:
    assert '@router.post("/{request_id}/repair-intents", status_code=201)' in API
    assert "tenant_id=request.tenant_id" in API


def test_extension_reservation_requires_authoritative_evidence() -> None:
    reserve = API.split('@router.post("/reserve-extension")', 1)[1].split(
...
def test_adopt_and_webrtc_require_exact_reservation_binding() -> None:
    assert API.count("EXTENSION_RESERVATION_MISMATCH") >= 2
    assert "TelephonyExtensionReservation.extension == int(body.extension)" in API
    assert "AgentProvisioningRequest.id == body.provisioning_request_id" in API
    assert 'reservation.state = "DISABLED_READY"' in API
