...
def test_postgresql_observability_contracts_are_implemented_read_only():
    expected = {
        "backup-catalog",
        "restore-rehearsal",
        "tls-security",
    ...
    assert expected <= set(READBACKS)
    assert {READBACKS[name]["state"] for name in expected} == {
        "IMPLEMENTED_READ_ONLY"
    }
    assert all(READBACKS[name]["method"] == "GET" for name in expected)
    assert all(
        READBACKS[name]["path"].startswith("/internal/v1/observability/")
